...
def test_soap_centers() -> None:
    """Test the consistency of SOAP calculations on specific centers.

    This test verifies that the SOAP calculation yields similar
    values as a control calculation at different r_cut using a specific
    selection for the centers. The calculation of SOAP is influenced by the
    architecture of the machine it's run on. As a result, the values of the
    SOAP components might exhibit minor variations. To disregard these
    differences, the function np.allclose() is employed.
